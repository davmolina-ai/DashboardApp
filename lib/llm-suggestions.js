...
async function generateAiRuleSuggestions(context) {
  ...
  const config = llmConfig();
  const requestUrl = buildChatCompletionsUrl(config.endpoint, config.apiVersion);
...
function buildPromptPayload(context) {
  const guidelineSources = context.guidelineSources || [];
  return {
    ...
    sample_rows: context.previewRows,
    guideline_templates: context.curatedSuggestions.map((rule) => ({
      name: rule.name,
...
function buildChatCompletionsUrl(endpoint, apiVersion) {
  ...
  if (normalized.includes("/api/projects/")) {
    const base = normalized.split("/api/projects/")[0];
    return `${base}/models/chat/completions?api-version=${encodeURIComponent(apiVersion)}`;
  }
  if (normalized.endsWith("/models")) {
    return `${normalized}/chat/completions?api-version=${encodeURIComponent(apiVersion)}`;
  }
  return `${normalized}/models/chat/completions?api-version=${encodeURIComponent(apiVersion)}`;
}
