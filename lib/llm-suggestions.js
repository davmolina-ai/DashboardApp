...
function llmConfig(provider = "phi") {
  const normalizedProvider = String(provider || "phi").toLowerCase();
  if (normalizedProvider === "deepseek") {
    return {
      provider: "deepseek",
      providerLabel: "DeepSeek",
      endpoint: process.env.AZURE_OPENAI_DEEPSEEK_ENDPOINT || "",
      apiKey: process.env.AZURE_OPENAI_DEEPSEEK_API_KEY || "",
      deployment: process.env.AZURE_OPENAI_DEEPSEEK_DEPLOYMENT || "",
      apiVersion: process.env.AZURE_OPENAI_DEEPSEEK_API_VERSION || process.env.AZURE_OPENAI_API_VERSION || "2024-05-01-preview"
    };
  }
...
function isLlmConfigured(provider = "phi") {
  const config = llmConfig(provider);
  return Boolean(config.endpoint && config.apiKey && config.deployment);
}

async function generateAiRuleSuggestions(context, options = {}) {
  const provider = options.provider || "phi";
  if (!isLlmConfigured(provider)) {
    return {
      ...
      meta: {
        ...
        used: false,
        provider,
        providerLabel: llmConfig(provider).providerLabel,
  ...
  const suggestions = suggestionItems
    .map((suggestion, index) =>
      normalizeAiSuggestion(suggestion, context, index, {
        deployment: config.deployment,
        providerLabel: config.providerLabel
      })
    )
...
function buildChatCompletionsUrls(endpoint, apiVersion) {
  ...
  if (/openai\.azure\.com$/i.test(base) || /services\.ai\.azure\.com$/i.test(base)) {
...
function normalizeAiSuggestion(suggestion, context, index, modelMeta = {}) {
  ...
  return {
    ...
    definition,
    suggestionOrigin: "ai",
    suggestionModel: modelMeta.deployment || "",
    suggestionProviderLabel: modelMeta.providerLabel || "Azure AI"
  };
}
...
function normalizeDefinition(definition, profile) {
  ...
  const normalizedRoot = normalizeNode(definition.root, true);
  const root = coerceRootGroup(normalizedRoot, definition);
  if (!root) {
    return null;
  }
...
function coerceRootGroup(root, definition) {
  if (root?.type === "group") {
    return root;
  }
  ...
  const fallbackChildren = [
    root,
    ...normalizeFallbackChildren(definition.conditions),
    ...normalizeFallbackChildren(definition.criteria),
    ...normalizeFallbackChildren(definition.rules),
    ...normalizeFallbackChildren(definition.children)
  ].filter(Boolean);
...
function normalizeFallbackChildren(value) {
  if (!Array.isArray(value)) {
    return [];
  }
  return value.map((child) => normalizeNode(child)).filter(Boolean);
}
...
function normalizeNode(node, isRoot = false) {
  ...
  if ((node.type === "condition" || typeof node.field === "string") && typeof node.field === "string") {
