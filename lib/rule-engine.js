...
// instead of a draft order, sending a communication - a message to provider
...
function buildServiceRequest(recommendation, dashboardName) {
  ...
  return {
    ...
    status: "completed",
    priority: "routine",
    category: [{ text: "Population health" }],
    sender: { display: "Dashboard-to-Orders Application" },
    recipient: [{ display: recommendation.provider }],
    subject: {
    ...
    payload: [
      {
        contentString: `Patient ${recommendation.patientName} is due for: ${recommendation.gapReason}. Recommended action: ${recommendation.orderInput.display}.`
      }
    ]
  };
}
